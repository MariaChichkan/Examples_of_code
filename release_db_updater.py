...
from utils import count_teams_and_features
...
class UpdateReleaseDB(DBConnector):
    ...
    def update_app_quantity(self, release_lst):
        df_app_quantity = count_teams_and_features(self.session, release_lst)['release_info']
        df_app_quantity['features_psi_ready'] = 0
        df_app_quantity['features_psi_notallowed'] = 0
        df_app_quantity['features_psi_passed'] = 0
        df_app_quantity['updated'] = pd.to_datetime('today').date()
        df_app_quantity['updated'] = df_app_quantity['updated'].astype(str)
        self.insert_db_data(ApplicationQuantity, df_app_quantity, update=True)

        # try:

        #     insert_stmt = insert(ApplicationQuantity).values(df_app_quantity.to_dict(orient='records'))
        #     on_conflict_stmt = insert_stmt.on_duplicate_key_update(
        #         **{'features': insert_stmt.inserted.features, "teams": insert_stmt.inserted.teams, "updated": updated},
        #         status='U')
    ...
    def update_mp_web_db(self, project_data):
        ...
        self.update_app_quantity(release_lst)
