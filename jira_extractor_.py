...
class JiraIssues:
    def __init__(self, logger):
        self.jira_options = {'server': 'https://sbtatlas.sigma.sbrf.ru/jira', 'verify': False}
        self.login =  "login"
        self.password =  "password"
        self.logger = logger
        self.date_now = datetime.date.today()  # datetime.datetime.now()
        try:
            self.jira = JIRA(options=self.jira_options, basic_auth=(self.login, self.password), max_retries=1)

        except Exception as ex:
            self.logger.error(f'__JiraIssues__Error {str(ex)} while connecting to Jira')
            sys.exit(-1)

    def reconnect_jira(self):
        try:
            self.jira = JIRA(options=self.jira_options, basic_auth=(self.login, self.password), max_retries=1)
        except Exception as ex:
            self.logger.error(f'__JiraIssues__Error {str(ex)} while connecting to Jira')
    ...
    def get_issues(self, issue_type, subtasksparents=[], epics=[], linkedIssues=[], project=''):
        ...
        return issues
        # except Exception as ex:
        #     print(f'{issue_type_jql}{project_jql}{issuekeys_jql} ')
...
class JiraDataByProject(JiraIssues):
    ...
    def get_release_info():
        PATH = r'%s' % os.getcwd().replace('\\', '/')
        PATH_TO_JSON = PATH + '/releases-3.json'
        with open(PATH_TO_JSON) as json_file:
            data = json.load(json_file)
        df_json_data = pd.DataFrame.from_records(data['body'][0]['releases'])
        df_platform = df_json_data.sort_values(by='jira_platform')
        df_platform = df_platform.drop_duplicates(subset=['jira_platform'], keep="first")
        platform_dict = {row['jira_platform']: row['platform'] for key, row in df_platform.iterrows()}
        df_id_releases = df_json_data.sort_values(by='key')
        df_id_releases = df_id_releases.drop_duplicates(subset='key', keep="first")
    ...
    @staticmethod
    def collect_linked_issues(df_tab):
    ...
    def get_web_mp_data(self, project):
        self.logger.info(f'__JiraIssues__Info: Starting to upload Jira data for project {project}')
    ...
    def fill_dor_content(self, df_tab):
        df_tab['fields.description'] = df_tab['fields.description'].apply(self.normalize_text)
        df_tab['fields.description'] = df_tab['fields.description'].apply(self.remove_tables)
        df_tab['urls'] = df_tab['fields.description'].apply(self.extract_url)
        df_tab['fields.description'] = df_tab['fields.description'].apply(self.remove_extra_symbols)
        df_tab['fields.description_wo_url'] = df_tab['fields.description'].apply(self.remove_url)
        return df_tab
